...
impl DependencyAnalyzer {
    ...
    fn build_levels(&mut self, graph: &CompiledGraph) -> Result<()> {
        ...
        if total_nodes != actual_nodes {
            return Err(
                ExecutionError::NodeExecutionFailed("Graph contains cycles".to_string()).into(),
            );
        }
...
impl StateVersionManager {
    ...
    pub async fn rollback(&self, version_id: usize) -> Result<StateData> {
        ...
        let version = versions
            ...
            .ok_or_else(|| {
                ExecutionError::NodeExecutionFailed(format!("Version {} not found", version_id))
            })?;
    ...
    pub async fn execute_parallel(
        ...
        for level in 0..analyzer.num_levels() {
            if let Some(batch) = analyzer.get_parallel_batch(level) {
                ...
                match self
                    .execute_batch(batch, graph, &state, Arc::clone(&node_executor))
                    .await
                {
                    Ok(_) => {
                    ...
                    Err(e) => {
                        ...
                        let rollback_state = self.version_manager.rollback(initial_version).await?;
    ...
    async fn execute_batch(
        ...
        for node_id in batch {
            ...
            let permit = self.semaphore.clone().acquire_owned().await.map_err(|e| {
                ExecutionError::NodeExecutionFailed(format!("Failed to acquire semaphore: {}", e))
            })?;
        ...
        while let Some(result) = futures.next().await {
            ...
            match result {
                ...
                Ok(Err(resilience_err)) => {
                    return Err(ExecutionError::NodeExecutionFailed(format!(
                        "Resilience error: {}",
                        resilience_err
                    ))
                    .into());
...
mod tests {
    ...
    async fn test_parallel_execution() {
        ...
        let result = executor
            .execute_parallel(&graph, initial_state)
            .await
            .unwrap();
    ...
    async fn test_parallel_performance() {
        ...
        for i in 0..10 {
            ...
            graph
                .add_edge("__start__", &format!("node_{}", i), Edge::direct())
                .unwrap();
        }
        ...
        for i in 0..10 {
            graph
                .add_edge(&format!("node_{}", i), "__end__", Edge::direct())
                .unwrap();
        }
        ...
        let _ = executor
            .execute_parallel(&compiled, StateData::new())
            .await
            .unwrap();
