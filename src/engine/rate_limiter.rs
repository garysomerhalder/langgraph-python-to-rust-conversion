...
impl RateLimiter {
    ...
    pub async fn try_acquire(&self) -> Result<RateLimitPermit, RateLimitError> {
        ...
        if bucket.try_consume(1.0) {
            ...
            Err(RateLimitError::RateLimitExceeded(
                format!("Rate limit exceeded: {}/{} per {:?}", 
    ...
    pub async fn acquire_timeout(&self, timeout: Duration) -> Result<RateLimitPermit, RateLimitError> {
...
impl AdaptiveRateLimiter {
    ...
    pub async fn acquire(&self) -> Result<RateLimitPermit, RateLimitError> {
        ...
        if load > self.load_threshold {
            let extra_delay = Duration::from_millis(
                ((load - self.load_threshold) * 1000.0) as u64
            );
