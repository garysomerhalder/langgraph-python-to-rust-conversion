...
use crate::engine::human_in_loop::{
    ApprovalDecision, ExecutionHandle, HumanInLoopExecution, InterruptCallback, InterruptManager,
    InterruptMode,
};
...
impl ExecutionEngine {
    ...
    pub async fn execute(&self, graph: CompiledGraph, input: StateData) -> Result<StateData> {
        let execution_id = generate_execution_id();
        let context = self
            .create_context(graph, input, execution_id.clone())
            .await?;
    ...
    pub async fn execute_until(
        ...
        let mut context = self
            .create_context(graph, input, execution_id.to_string())
            .await?;
    ...
    pub async fn resume_from(
        ...
        let context = self
            .create_context(graph, snapshot.state.clone(), execution_id.clone())
            .await?;
    ...
    pub async fn resume_from_node(
        ...
        let context = self
            .create_context((*graph).clone(), state, execution_id.clone())
            .await?;
    ...
    pub async fn start_execution(&self, graph: CompiledGraph, input: StateData) -> Result<Uuid> {
        let execution_id = Uuid::new_v4();
        let context = self
            .create_context(graph, input, execution_id.to_string())
            .await?;
    ...
    async fn execute_graph(&self, context: &ExecutionContext) -> Result<StateData> {
        ...
        for node_id in execution_order {
            ...
            let node = context.graph.graph().get_node(&node_id).ok_or_else(|| {
                ExecutionError::NodeExecutionFailed(format!("Node not found: {}", node_id))
            })?;
    ...
    async fn execute_graph_streaming(
        ...
        for node_id in execution_order {
            ...
            tx.send(msg)
                .await
                .map_err(|e| ExecutionError::MessageError(e.to_string()))?;
