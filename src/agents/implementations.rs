...
impl ChainOfThoughtAgent {
    ...
    fn extract_string_params(&self, state: &StateData) -> Result<Value> {
        ...
        let input = state.get("text").and_then(|v| v.as_str()).unwrap_or("");
    ...
    async fn can_conclude(&self, steps: &[ReasoningStep], state: &StateData) -> Result<bool> {
        ...
        let observations_count = steps.iter().filter(|s| s.observation.is_some()).count();
...
impl Agent for ChainOfThoughtAgent {
    ...
    async fn process(&self, input: String, state: StateData) -> Result<StateData> {
        ...
        new_state.insert("reasoning_steps".to_string(), json!(reasoning_steps));
        ...
        if let Some(last_message) = conversation.messages.last() {
            if last_message.role == MessageRole::User {
                let reasoning_steps = self
                    .think_step_by_step(&last_message.content, &state)
                    .await?;
...
impl MemoryAgent {
    ...
    async fn consolidate_memories(&self) -> Result<()> {
        ...
        let memories_to_consolidate: Vec<_> =
            stm.iter().filter(|m| m.access_count > 3).cloned().collect();
        drop(stm);
...
mod tests {
    ...
    async fn test_chain_of_thought_agent() {
        ...
        let result = agent
            .process("Calculate the sum of the numbers".to_string(), state)
            .await
            .unwrap();
    ...
    async fn test_react_agent() {
        ...
        let result = agent
            .process("Transform the text to uppercase".to_string(), state)
            .await
            .unwrap();
    ...
    async fn test_memory_agent() {
        ...
        // Store some memories
        let result1 = agent
            .process("The capital of France is Paris".to_string(), state.clone())
            .await
            .unwrap();
        ...
        let result2 = agent
            .process("What about France?".to_string(), state)
            .await
            .unwrap();
