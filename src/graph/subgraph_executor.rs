...
impl RecursiveSubgraphExecutor {
    ...
    pub async fn execute_recursive(
        ...
        if *depth >= self.max_depth {
            return Err(crate::graph::GraphError::RuntimeError(format!(
        ...
        *depth += 1;
        ...
        let result = self
            .base_executor
            .execute_subgraph(subgraph_id, subgraph, parent_state)
            .await;
...
impl ParallelSubgraphExecutor {
    ...
    pub async fn execute_parallel(
        ...
        for (i, (subgraph_id, subgraph)) in subgraphs.iter().enumerate() {
            ...
            let handle =
                tokio::spawn(
                    async move { executor_clone.execute_subgraph(&id, &graph, &state).await },
                );
    ...
    fn merge_results(&self, results: Vec<StateData>) -> Result<StateData> {
        ...
        match &self.merge_strategy {
            MergeStrategy::FirstSuccess => Ok(results.into_iter().next().unwrap()),
            ...
            MergeStrategy::Custom(merger) => Ok(merger(&results)),
            ...
            MergeStrategy::Voting => {
                ...
                for (key, value_votes) in vote_map {
                    if let Some((winning_value, _)) =
                        value_votes.iter().max_by_key(|(_, count)| *count)
                    {
...
mod tests {
    ...
    async fn test_subgraph_executor() {
        ...
        let result = executor
            .execute_subgraph("test", &subgraph, &parent_state)
            .await;
