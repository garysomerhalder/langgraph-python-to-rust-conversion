...
pub use conditional::{ConditionalBranch, ConditionalRouter};
pub use edge::{ConditionalEdge, Edge, EdgeType};
pub use node::{Node, NodeFunction, NodeType};
pub use state_graph::{StateConditionalEdge, StateGraphManager};
pub use subgraph::{
    PassthroughMapper, RecursiveSubgraph, SelectiveMapper, StateMapper, Subgraph, SubgraphBuilder,
};
pub use subgraph_executor::{
    ConditionalSubgraphExecutor, IsolationStrategy, MergeStrategy, ParallelSubgraphExecutor,
    RecursiveSubgraphExecutor, SubgraphExecutor,
};
...
impl CompiledGraph {
    ...
    pub async fn stream(
        &self,
        input: StateData,
    ) -> Result<impl futures::Stream<Item = Result<StateData>>> {
