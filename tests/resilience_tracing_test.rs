use langgraph::engine::executor::{
    ExecutionContext, ExecutionEngine, ExecutionMetadata, ExecutionStatus,
};
...
use langgraph::engine::tracing::{SpanEvent, SpanStatus, Tracer};
use langgraph::graph::{GraphBuilder, Node, NodeType};
...
async fn test_circuit_breaker() {
    ...
    let result = manager
        .execute_with_resilience(|| async { Ok::<_, std::io::Error>("Success") })
        .await;
    assert!(result.is_ok());
}
...
async fn test_distributed_tracing() {
    ...
    // Add tags
    parent_span
        .add_tag("user_id".to_string(), "123".to_string())
        .await;
    parent_span
        .add_tag("request_id".to_string(), "abc-def".to_string())
        .await;
    ...
    // Add child span tags
    child_span
        .add_tag("db_query".to_string(), "SELECT * FROM users".to_string())
        .await;
...
async fn test_resilient_node_execution() {
    ...
    let result = retry_executor
        .execute_with_retry(&node, &mut state.values, &context)
        .await;
    assert!(result.is_ok());
