use langgraph::checkpoint::{Checkpoint, Checkpointer, InMemoryCheckpointer};
...
use langgraph::stream::{
    ChannelReceiver, ChannelRegistry, ChannelSender, ChannelType, DefaultStreamingEngine,
    FlowController, RateLimiter, StreamingEngine,
};
use serde_json::json;
use std::sync::Arc;
use tokio;
...
async fn test_complete_workflow_execution() {
    ...
    let builder = GraphBuilder::new("customer_support_workflow")
        ...
        .add_node(
            "handle_complaint",
            NodeType::Agent("complaint_handler".to_string()),
        )
        .add_node(
            "handle_inquiry",
            NodeType::Agent("inquiry_handler".to_string()),
        )
        .add_node(
            "generate_response",
            NodeType::Agent("response_gen".to_string()),
        )
...
async fn test_state_checkpointing() {
    ...
    state
        .values
        .insert("session_id".to_string(), json!("session_456"));
    state
        .values
        .insert("messages".to_string(), json!(["Hello", "How can I help?"]));
    ...
    assert_eq!(
        loaded_checkpoint.state.values.get("user_id"),
        state.values.get("user_id")
    );
    assert_eq!(
        loaded_checkpoint.state.values.get("session_id"),
        state.values.get("session_id")
    );
    assert_eq!(
        loaded_checkpoint.state.values.get("messages"),
...
async fn test_error_handling() {
    let builder = GraphBuilder::new("error_handling_workflow")
        ...
        .add_node(
            "error_handler",
            NodeType::Agent("error_handler".to_string()),
        )
...
async fn test_channel_communication() {
    ...
    registry
        .create_channel("data_channel".to_string(), ChannelType::Mpsc(100))
        .await
        .unwrap();

    registry
        .create_channel("control_channel".to_string(), ChannelType::Mpsc(10))
        .await
        .unwrap();

    // Test data flow with simple types
    let data_sender: ChannelSender<String> = registry.get_sender("data_channel").await.unwrap();
    let mut data_receiver: ChannelReceiver<String> =
        registry.get_receiver("data_channel").await.unwrap();
...
async fn test_concurrent_state_updates() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            state_guard
                .values
                .insert(format!("task_{}", i), json!(format!("value_{}", i)));
        });
        handles.push(handle);
    }
