...
use langgraph::state::{State, GraphState};
...
async fn test_streaming_engine() {
    ...
    let mut stream = engine.stream_execution(graph_arc, initial_state)
        .await
        .unwrap();
...
async fn test_channel_registry() {
    ...
    registry.create_channel(
        "test_broadcast".to_string(),
        ChannelType::Broadcast(10)
    ).await.unwrap();
...
async fn test_stream_collectors() {
    ...
    map_collector.collect_item(("key1".to_string(), "value1".to_string())).await.unwrap();
    map_collector.collect_item(("key2".to_string(), "value2".to_string())).await.unwrap();
