...
use langgraph::agents::{
    Agent, AgentMemory, CoordinationStrategy, MultiAgentCoordinator, ReasoningAgent,
    ReasoningStrategy,
};
use langgraph::graph::{
    ConditionalBranch, ConditionalRouter, GraphBuilder, NodeType, PassthroughMapper,
    SelectiveMapper, Subgraph, SubgraphBuilder,
};
use langgraph::state::advanced::{SnapshotManager, StateDiff, VersionedStateManager};
use langgraph::state::{GraphState, StateData};
use langgraph::tools::{
    FunctionTool, HttpTool, Tool, ToolChain, ToolContext, ToolParameter, ToolRegistry, ToolResult,
    ToolSpec,
};
...
async fn test_subgraph_composition() {
    ...
    subgraph = subgraph
        .with_input_mapper(passthrough.clone())
    ...
    let selective = SelectiveMapper::new().add_mapping("input_key", "mapped_key");
...
async fn test_agent_reasoning() {
    ...
    agent
        .observe(observation.clone(), &state.values)
        .await
        .unwrap();
...
async fn test_multi_agent_coordination() {
    ...
    let result = coordinator
        .execute(initial_state.values.clone())
        .await
        .unwrap();
...
async fn test_state_versioning() {
    ...
    let v1 = manager
        .create_version(state1.clone(), Some("Initial version".to_string()))
        .await
        .unwrap();
    assert_eq!(v1, 1);
    ...
    let v2 = manager
        .create_version(state2, Some("Modified data".to_string()))
        .await
        .unwrap();
    assert_eq!(v2, 2);
...
async fn test_state_snapshots() {
    ...
    for i in 1..=3 {
        ...
        snapshot_manager
            .create_snapshot(format!("snap_{}", i), state, i as u64, None)
            .await
            .unwrap();
    }
...
fn test_subgraph_builder() {
    ...
    let subgraph = Subgraph::new("sub1", sub_compiled).with_isolation(true);
