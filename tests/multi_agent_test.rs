...
use langgraph::{
    agents::{
        ...
        OrchestratorAgent, SpecializedAgent,
    },
    Result,
};
use serde_json::json;
...
async fn test_data_analysis_workflow() {
    ...
    let stages = result["stages"].as_array().unwrap();
    let agent_roles: Vec<String> = stages.iter()
...
async fn test_parallel_agent_execution() {
    ...
    let tasks = vec![
        (AgentRole::Research, json!({"topic": "microservices architecture"})),
        (AgentRole::Security, json!({"name": "vulnerability_scan"})),
        (AgentRole::Data, json!({"name": "performance_metrics"})),
        (AgentRole::Product, json!({"features": ["feature_a", "feature_b"]})),
    ];
...
async fn test_orchestrator_agent_delegation() {
    ...
    if let Some(orchestrator) = system.get_agent(AgentRole::Orchestrator) {
        let response = orchestrator.process_message(delegation_message).await.unwrap();
...
async fn test_agent_capability_matching() {
    ...
    assert!(capabilities.skills.contains(&"information_gathering".to_string()));
    assert!(capabilities.skills.contains(&"technology_evaluation".to_string()));
    ...
    assert!(arch_capabilities.skills.contains(&"system_design".to_string()));
    assert!(arch_capabilities.skills.contains(&"scalability_planning".to_string()));
