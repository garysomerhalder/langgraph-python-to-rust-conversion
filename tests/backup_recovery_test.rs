...
use langgraph::backup::{BackupManager, BackupType, FileBackupStorage, RetentionPolicy};
...
async fn test_backup_manager_full_backup_creation() {
    ...
    let checkpoint_data = HashMap::from([
        (
            "state".to_string(),
            json!({"counter": 42, "status": "active"}),
        ),
        (
            "metadata".to_string(),
            json!({"version": "1.0", "created_by": "test"}),
        ),
    ]);
    ...
    let checkpoint_id = checkpointer
        .save(thread_id, checkpoint_data.clone(), metadata.clone(), None)
        .await
    ...
    let backup_id = backup_manager
        .create_full_backup(&checkpointer, Some("test_backup"))
        .await
...
async fn test_backup_manager_full_restore_operation() {
    ...
    let original_data = HashMap::from([
        (
            ...
            json!({"step": 5, "completed": ["init", "process", "validate"]}),
        ),
        (
            "user_context".to_string(),
            json!({"user_id": "test_user", "session": "abc123"}),
        ),
    ]);
    let metadata = HashMap::new();

    source_checkpointer
        .save(thread_id, original_data.clone(), metadata, None)
        .await
        .expect("Failed to save original checkpoint");

    // Create backup
    let backup_manager = BackupManager::new(Box::new(FileBackupStorage::new("./backups")));
    let backup_id = backup_manager
        .create_full_backup(&source_checkpointer, Some("restore_test"))
        .await
        .expect("Failed to create backup");

    // Restore to target checkpointer
    let restore_result = backup_manager
        .restore_backup(&backup_id, &target_checkpointer)
        .await
    ...
    let restored_data = target_checkpointer
        .load(thread_id, None)
        .await
        .expect("Failed to load restored data")
        .expect("Restored data should exist");

    assert_eq!(
        restored_data.0.get("workflow_state"),
        original_data.get("workflow_state")
    );
    assert_eq!(
        restored_data.0.get("user_context"),
        original_data.get("user_context")
    );
}
...
async fn test_backup_manager_incremental_backup_chain() {
    ...
    checkpointer
        .save(thread_id, initial_data, HashMap::new(), None)
        .await
        .expect("Failed to save initial checkpoint");

    let full_backup_id = backup_manager
        .create_full_backup(&checkpointer, Some("incremental_base"))
        .await
        .expect("Failed to create full backup");
    ...
    checkpointer
        .save(thread_id, updated_data, HashMap::new(), None)
        .await
        .expect("Failed to save updated checkpoint");

    // Create incremental backup
    let incremental_backup_id = backup_manager
        .create_incremental_backup(&checkpointer, &full_backup_id, Some("incremental_1"))
        .await
    ...
    checkpointer
        .save(thread_id, final_data, HashMap::new(), None)
        .await
        .expect("Failed to save final checkpoint");

    let incremental_backup_2_id = backup_manager
        .create_incremental_backup(&checkpointer, &incremental_backup_id, Some("incremental_2"))
        .await
        .expect("Failed to create second incremental backup");

    // Restore from incremental backup chain
    let target_checkpointer = MemoryCheckpointer::new();
    let restore_result = backup_manager
        .restore_incremental_chain(&incremental_backup_2_id, &target_checkpointer)
        .await
        .expect("Failed to restore incremental chain");
...
async fn test_backup_manager_cross_backend_restore() {
    ...
    let test_data = HashMap::from([
        ...
        (
            ...
            json!({"data": "cross_backend_data", "version": 2}),
        ),
    ]);

    memory_checkpointer
        .save(thread_id, test_data.clone(), HashMap::new(), None)
        .await
        .expect("Failed to save to memory backend");

    // Create backup from memory backend
    let backup_manager = BackupManager::new(Box::new(FileBackupStorage::new("./backups")));
    let backup_id = backup_manager
        .create_full_backup(&memory_checkpointer, Some("cross_backend_test"))
        .await
        .expect("Failed to create backup from memory backend");
    ...
    let restore_result = backup_manager
        .restore_backup(&backup_id, &target_checkpointer)
        .await
        .expect("Failed to restore to target backend");
    ...
    let restored_data = target_checkpointer
        .load(thread_id, None)
        .await
        .expect("Failed to load from target backend")
        .expect("Data should exist in target backend");

    assert_eq!(
        restored_data.0.get("migration_test"),
        test_data.get("migration_test")
    );
    assert_eq!(
        restored_data.0.get("source_backend"),
        test_data.get("source_backend")
    );
}
...
async fn test_backup_manager_retention_policy() {
    ...
    let backup_manager =
        BackupManager::new(Box::new(backup_storage)).with_retention_policy(retention_policy);

    let thread_id = "retention_test_thread";
    let test_data = HashMap::from([("retention_test".to_string(), json!(true))]);

    checkpointer
        .save(thread_id, test_data, HashMap::new(), None)
        .await
        .expect("Failed to save checkpoint");
    ...
    for i in 0..5 {
        let backup_id = backup_manager
            .create_full_backup(&checkpointer, Some(&format!("retention_test_{}", i)))
            .await
            .expect("Failed to create backup");
    ...
    for old_backup_id in &backup_ids[0..2] {
        ...
        assert!(
            backup_info.is_err(),
            "Old backup should no longer be accessible"
        );
    }
}
...
async fn test_backup_manager_backup_verification() {
    ...
    let backup_id = backup_manager
        .create_full_backup(&checkpointer, Some("verification_test"))
        .await
        .expect("Failed to create backup");

    // Verify backup integrity
    let verification_result = backup_manager
    ...
    let detailed_verification = backup_manager
        .verify_backup_detailed(&backup_id, true)
        .await
        .expect("Failed to perform detailed verification");
...
async fn test_backup_manager_disaster_recovery_scenario() {
    ...
    for (i, thread_id) in threads.iter().enumerate() {
        ...
        primary_checkpointer
            .save(thread_id, data.clone(), HashMap::new(), None)
            .await
            .expect("Failed to save workflow checkpoint");

        original_data.insert(thread_id.to_string(), data);
    }

    // Create disaster recovery backup
    let disaster_backup_id = backup_manager
        .create_disaster_recovery_backup(&primary_checkpointer, "disaster_scenario")
        .await
        .expect("Failed to create disaster recovery backup");
    ...
    let recovery_result = backup_manager
        .disaster_recovery_restore(&disaster_backup_id, &recovered_checkpointer)
        .await
        .expect("Failed to perform disaster recovery");
    ...
    for thread_id in &threads {
        let recovered_data = recovered_checkpointer
            .load(thread_id, None)
            .await
            .expect("Failed to load recovered workflow")
            .expect("Recovered workflow should exist");

        let original = original_data.get(*thread_id).unwrap();
        assert_eq!(
            recovered_data.0.get("workflow_id"),
