...
use langgraph::graph::{StateGraph, Node, NodeType, Edge};
...
async fn test_parallel_node_execution() {
    ...
    for i in 1..=4 {
        graph.add_edge("__start__", &format!("process_{}", i), Edge::direct()).unwrap();
        graph.add_edge(&format!("process_{}", i), "__end__", Edge::direct()).unwrap();
    }
    ...
    let result = executor.execute_parallel(&compiled, initial_state.clone()).await.unwrap();
...
async fn test_state_versioning() {
    ...
    assert_eq!(retrieved_v1.get("data"), Some(&serde_json::json!("initial")));
    
    let retrieved_v2 = version_manager.rollback(v2_id).await.unwrap();
    assert_eq!(retrieved_v2.get("data"), Some(&serde_json::json!("modified")));
    assert_eq!(retrieved_v2.get("new_field"), Some(&serde_json::json!("added")));
...
async fn test_dependency_resolution() {
    ...
    graph.add_edge("__start__", "task_a", Edge::direct()).unwrap();
    ...
    let result = executor.execute_parallel(&compiled, initial_state).await.unwrap();
...
async fn test_execution_metrics() {
    ...
    let _result = executor.execute_parallel(&compiled, initial_state).await.unwrap();
...
async fn test_h_cycle_complete_flow() {
    ...
    // Connect H structure
    graph.add_edge("__start__", "left_1", Edge::direct()).unwrap();
    graph.add_edge("__start__", "right_1", Edge::direct()).unwrap();
    
    graph.add_edge("left_1", "left_2", Edge::direct()).unwrap();
    graph.add_edge("right_1", "right_2", Edge::direct()).unwrap();
