...
use langgraph::engine::parallel_executor::{ExecutionMetrics, ParallelExecutor};
use langgraph::engine::rate_limiter::RateLimiter;
use langgraph::engine::resilience::{CircuitBreakerConfig, ResilienceManager, RetryConfig};
use langgraph::engine::tracing::{TraceContext, TracingManager};
use langgraph::graph::{Edge, GraphBuilder, Node, NodeType, StateGraph};
use langgraph::state::validation::{StateValidator, ValidationRuleBuilder, ValueType};
use langgraph::state::{GraphState, StateData};
use langgraph::utils::object_pool::{pools, ObjectPool};
use serde_json::json;
...
fn benchmark_state_operations(c: &mut Criterion) {
    ...
    group.bench_function("state_bulk_update", |b| {
        let mut state = GraphState::new();
        let updates: StateData = (0..100).map(|i| (format!("key_{}", i), json!(i))).collect();
...
fn benchmark_metrics(c: &mut Criterion) {
    ...
    group.bench_function("metrics_export", |b| {
        ...
        for i in 0..100 {
            collector
                .record_execution_time(&format!("node_{}", i), Duration::from_millis(i as u64));
        }
...
fn benchmark_resilience(c: &mut Criterion) {
    ...
    group.bench_function("circuit_breaker_closed", |b| {
        let manager = ResilienceManager::new().with_circuit_breaker(5, Duration::from_secs(60));

        b.to_async(&rt).iter(|| async {
            let result = manager
                .execute("test", || async { Ok::<_, Box<dyn std::error::Error>>(42) })
                .await;
            black_box(result)
        })
    });
...
fn create_linear_graph(size: usize) -> StateGraph {
    ...
    if size > 0 {
        ...
        for i in 1..size {
            ...
            builder = builder.add_edge(format!("node_{}", i - 1), format!("node_{}", i));
        }
