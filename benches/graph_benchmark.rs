...
fn create_simple_graph() -> StateGraph {
    ...
    graph.add_edge("process", "validate", Edge::direct()).unwrap();
    graph.add_edge("validate", "__end__", Edge::direct()).unwrap();
...
fn create_complex_graph() -> StateGraph {
    ...
    graph.add_edge("__start__", "node_0", Edge::direct()).unwrap();
...
fn benchmark_graph_compilation(c: &mut Criterion) {
    ...
    c.bench_function("compile_simple_graph", |b| {
        b.iter(|| {
            simple_graph.clone().compile()
        })
    });
    
    c.bench_function("compile_complex_graph", |b| {
        b.iter(|| {
            complex_graph.clone().compile()
        })
    });
}
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                for i in 0..5 {
                    ...
                    graph.add_edge("__start__", &format!("parallel_{}", i), Edge::direct()).unwrap();
                    graph.add_edge(&format!("parallel_{}", i), "__end__", Edge::direct()).unwrap();
                }
