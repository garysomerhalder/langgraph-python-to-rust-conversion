...
use langgraph::graph::{ConditionalBranch, ConditionalRouter, GraphBuilder, NodeType};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let graph = GraphBuilder::new("decision_workflow")
        ...
        .add_node(
            "medium_priority",
            NodeType::Agent("medium_handler".to_string()),
        )
    ...
    let route1 = router.route_with_branches(
        &state1,
        vec![
            high_branch.clone(),
            medium_branch.clone(),
    ...
    let route2 = router.route_with_branches(
        &state2,
        vec![
            high_branch.clone(),
            medium_branch.clone(),
    ...
    let route3 = router.route_with_branches(&state3, vec![high_branch, medium_branch, low_branch]);
