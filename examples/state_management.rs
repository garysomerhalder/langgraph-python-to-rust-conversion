...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let v1 = version_manager
        .create_version(v1_state.clone(), Some("Initial version".to_string()))
        .await?;
    ...
    let v2 = version_manager
        .create_version(v2_state.clone(), Some("Added analytics".to_string()))
        .await?;
    ...
    let v3 = version_manager
        .create_version(v3_state, Some("Performance optimization".to_string()))
        .await?;
    ...
    snapshot_manager
        .create_snapshot("checkpoint_gamma".to_string(), snapshot_state, 3, None)
        .await?;
