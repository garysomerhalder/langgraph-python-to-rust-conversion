...
use langgraph::agents::{Agent, AgentMemory, ReasoningAgent, ReasoningStrategy};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    research_agent
        .observe(research_observation.clone(), &state.values)
        .await?;
    ...
    analysis_agent
        .observe(analysis_observation, &state.values)
        .await?;
    ...
    decision_agent
        .observe(decision_observation, &state.values)
        .await?;
    ...
    let result = decision_agent
        .act(&final_decision, &tools, &mut final_state)
        .await?;
    ...
    println!("\n🧠 Agent Memory States:");
    println!(
        "  Research Agent: {} short-term entries",
        research_agent.memory().short_term.len()
    );
    println!(
        "  Analysis Agent: {} short-term entries",
        analysis_agent.memory().short_term.len()
    );
    println!(
        "  Decision Agent: {} short-term entries",
        decision_agent.memory().short_term.len()
    );
